...
resource "aws_s3_bucket" "lambda_code_store" {
  bucket              = "${var.project}-${var.env}-lambda-code-store-01"
  object_lock_enabled = false
  force_destroy       = true

  tags = {
    Name = "${var.project}-${var.env}-lambda-code-store-01"
  }
}

# ACL無効化
resource "aws_s3_bucket_ownership_controls" "lambda_code_store" {
  bucket = aws_s3_bucket.lambda_code_store.id
  rule {
    object_ownership = "BucketOwnerEnforced"
  }
}
...
resource "aws_s3_bucket_public_access_block" "lambda_code_store" {
  bucket                  = aws_s3_bucket.lambda_code_store.id
  block_public_acls       = true
  block_public_policy     = true
  ignore_public_acls      = true
...
resource "aws_s3_bucket_server_side_encryption_configuration" "lambda_code_store" {
  bucket = aws_s3_bucket.lambda_code_store.id
  rule {
    apply_server_side_encryption_by_default {
      sse_algorithm = "AES256"
